...
export default function Footer() {
  return (
    <footer data-testid="site-footer" className="bg-baume-burgundy text-baume-white mt-24">
      <div className="baume-container py-16 md:py-24 grid grid-cols-2 md:grid-cols-12 gap-8 md:gap-12">
        <div className="col-span-2 md:col-span-4">
          ...
          <p className="mt-4 text-[14px] leading-[22px] text-baume-white/75 max-w-[280px]">
            Prenez soin de votre intimité à chaque étape de votre vie. Produits choisis avec exigence,
            conseils humains, boutique à Genève.
          </p>
          <div className="mt-8 flex items-center gap-4">
            <a href="https://instagram.com" aria-label="Instagram" className="hover:text-baume-taupe transition-colors">
              <Instagram className="h-5 w-5" />
            </a>
            <a href="https://facebook.com" aria-label="Facebook" className="hover:text-baume-taupe transition-colors">
        ...
        <div className="col-span-1 md:col-span-3">
          <p className="text-[12px] uppercase tracking-[0.2em] text-baume-taupe mb-4">Par besoin</p>
          <ul className="space-y-2 text-[14px]">
            {NEEDS.slice(0, 6).map((n) => (
              <li key={n.slug}>
                <Link to={`/shop/besoin/${n.slug}`} className="text-baume-white/80 hover:text-baume-white">
                  {n.name}
                </Link>
              </li>
            ))}
        ...
        <div className="col-span-1 md:col-span-3">
          <p className="text-[12px] uppercase tracking-[0.2em] text-baume-taupe mb-4">Par produit</p>
          <ul className="space-y-2 text-[14px]">
            {PRODUCT_CATS.slice(0, 6).map((c) => (
              <li key={c.slug}>
                <Link to={`/shop/produit/${c.slug}`} className="text-baume-white/80 hover:text-baume-white">
                  {c.name}
                </Link>
              </li>
            ))}
        ...
        <div className="col-span-2 md:col-span-2">
          <p className="text-[12px] uppercase tracking-[0.2em] text-baume-taupe mb-4">Boutique</p>
          ...
          <p className="mt-4 text-[14px] text-baume-white/80 flex items-center gap-2">
            ...
            <a href="mailto:bonjour@baume-shop.com" className="hover:text-baume-white">
