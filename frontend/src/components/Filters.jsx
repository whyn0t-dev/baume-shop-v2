...
function FilterPanel({ filters, setFilters, needs = [], cats = [], onReset }) {
  ...
  return (
    <div className="space-y-8">
      {needs.length > 0 && (
        <div>
          <p className="text-[12px] uppercase tracking-[0.18em] text-baume-burgundy font-semibold mb-3">Besoin</p>
          <ul className="space-y-2">
            {needs.map((n) => (
              <li key={n.slug} className="flex items-center gap-2">
                ...
                />
                <label htmlFor={`need-${n.slug}`} className="text-[14px] text-baume-charcoal cursor-pointer">{n.name}</label>
              </li>
            ))}
          </ul>
        </div>
      ...
      {cats.length > 0 && (
        <div>
          <p className="text-[12px] uppercase tracking-[0.18em] text-baume-burgundy font-semibold mb-3">Catégorie</p>
          <ul className="space-y-2">
            {cats.map((c) => (
              <li key={c.slug} className="flex items-center gap-2">
                <Checkbox
                  ...
                  className="border-baume-border data-[state=checked]:bg-baume-burgundy data-[state=checked]:border-baume-burgundy"
                />
                <label htmlFor={`cat-${c.slug}`} className="text-[14px] text-baume-charcoal cursor-pointer">{c.name}</label>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div>
        <p className="text-[12px] uppercase tracking-[0.18em] text-baume-burgundy font-semibold mb-3">Flux</p>
      ...
      <div>
        <p className="text-[12px] uppercase tracking-[0.18em] text-baume-burgundy font-semibold mb-3">Usage</p>
        <div className="flex flex-wrap gap-2">
          {USAGE_OPTIONS.map((o) => (
            <button
              key={o.value}
      ...
      <div>
        <p className="text-[12px] uppercase tracking-[0.18em] text-baume-burgundy font-semibold mb-3">Taille</p>
        <div className="flex flex-wrap gap-2">
          {SIZES.map((s) => (
            <button
              key={s}
      ...
      <div className="flex items-center gap-2">
        <Checkbox
          ...
          onCheckedChange={(v) => setFilters((f) => ({ ...f, available: v ? true : null }))}
          className="border-baume-border data-[state=checked]:bg-baume-burgundy data-[state=checked]:border-baume-burgundy"
        />
        <label htmlFor="available" className="text-[14px] text-baume-charcoal cursor-pointer">Uniquement disponibles</label>
      </div>
      <Button
        onClick={onReset}
        variant="ghost"
  ...
  return (
    <>
      ...
      <aside className="hidden lg:block sticky top-[96px] w-full max-w-[260px] h-max pr-4">
        <FilterPanel filters={filters} setFilters={setFilters} needs={needs} cats={cats} onReset={onReset} />
      </aside>
