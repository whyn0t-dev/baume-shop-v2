  ...
  return (
    <div data-testid="faq-page" className="bg-baume-ivory">
      ...
      <div className="baume-container py-10 md:py-14 max-w-[720px]">
        <p className="text-[12px] uppercase tracking-[0.2em] text-baume-burgundy font-semibold mb-3">Aide & informations</p>
        <h1 className="font-editorial text-[40px] md:text-[56px] leading-[1.05] text-baume-charcoal">Questions fréquentes</h1>
        <p className="mt-5 text-[17px] leading-[28px] text-baume-charcoal/70">Livraison, retours, retrait, ateliers — toutes les réponses essentielles.</p>
      ...
      <div className="baume-container pb-24 max-w-[720px]">
        <Accordion type="single" collapsible className="space-y-3">
          {filtered.map((f, i) => (
            <AccordionItem key={i} value={`item-${i}`} className="bg-baume-white border border-baume-border rounded-2xl px-5">
              <AccordionTrigger className="py-5 text-left font-editorial text-[20px] text-baume-charcoal hover:no-underline">
                {f.q}
              </AccordionTrigger>
              <AccordionContent className="pb-5 text-[15px] leading-[24px] text-baume-charcoal/80">{f.a}</AccordionContent>
            </AccordionItem>
          ))}
          {filtered.length === 0 && (
            <div className="rounded-2xl border border-baume-border bg-baume-white p-8 text-center">
              <p className="text-[15px] text-baume-charcoal/70">Aucune question trouvée. Essayez un autre mot-clé.</p>
            </div>
          )}
        </Accordion>
      </div>
